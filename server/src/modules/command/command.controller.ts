...
import { sendCommand, getCommandsForCity, cancelCommand, withdrawStationedSupport } from "./command.service";
...
export const sendCommandHandler = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (code === "CANNOT_ATTACK_ALLIANCE_MEMBER") return res.status(400).json({ error: code });
