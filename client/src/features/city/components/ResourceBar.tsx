...
export default function ResourceBar({ cityName, cityPoints, money, energy, ammo, capacity, moneyProd, energyProd, ammoProd, population, maxPopulation, onLogout, onSimulator, onReports, onMap, onRankings, onAlliance, onMessages, unreadReports = 0, unreadMessages = 0, ownedCities, activeCityId, onSwitchCity }: Props) {
  ...
  useClickOutside(cityMenuRef, cityMenuOpen, closeCityMenu);
  ...
  return (
    <div className="flex gap-6 px-4 py-1.5 bg-[#161b22] border-b border-[#30363d] shrink-0 text-sm text-[#c9d1d9]" style={{ fontVariantNumeric: "tabular-nums" }}>
      ...
      {(onSimulator || onRankings || onAlliance || onLogout) && (
        <div ref={menuRef} className="relative ml-auto shrink-0">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1 w-36 bg-[#161b22] border border-[#30363d] rounded shadow-lg z-50 overflow-hidden">
              ...
              {onAlliance && (
                <button
                  onClick={() => { setMenuOpen(false); onAlliance(); }}
                  className="block w-full text-left text-xs text-[#58a6ff] px-3 py-2 hover:bg-[#1c2129] cursor-pointer"
                >
                  Alliance
                </button>
              )}
              ...
              {(onRankings || onAlliance || onSimulator) && onLogout && <div className="h-px bg-[#30363d]" />}
              {onLogout && (
                <button
